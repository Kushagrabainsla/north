...
_BUILTIN_AGENTS_ROOT = Path(__file__).parent.parent.parent / "agents"
...
class {class_name}(AgenticLLMAgent):
    """Personal agent generated by North."""
'''
...
class CreateAgentTool(Tool):
    ...
    async def run(self, input: ToolInput) -> ToolOutput:
        ...
        if action == "list":
            return _list_agents(self._agents_dir)
        if action == "read":
            return _read_agent(input.params.get("name") or "", self._agents_dir)
        if action == "create":
    ...
    def _create(self, params: dict) -> ToolOutput:
        ...
        if not isinstance(raw_accepts, list) or not all(isinstance(item, str) for item in raw_accepts):
            return ToolOutput(success=False, error="'accepts' must be a list of routing keywords.")
        accepts = [item.strip() for item in raw_accepts if item.strip()]
        if not accepts:
            return ToolOutput(
                success=False,
        ...
        if self._agent_registry is not None and name in self._agent_registry.names():
            return ToolOutput(
                success=False,
                error=f"Agent '{name}' is already registered. Choose a different name.",
            )
        ...
        if agent_dir.exists():
            return ToolOutput(
                success=False,
                error=(f"Agent '{name}' already exists at {agent_dir}/. Use action='read' to inspect it."),
            )
...
def _agent_roots(agents_root: Path) -> list[Path]:
    """Return built-ins and the selected extension root without duplicates."""
    if agents_root == _BUILTIN_AGENTS_ROOT:
        return [_BUILTIN_AGENTS_ROOT]
    return [_BUILTIN_AGENTS_ROOT, agents_root]


def _list_agents(agents_root: Path = _BUILTIN_AGENTS_ROOT) -> ToolOutput:
    rows: list[dict] = []
    seen: set[str] = set()
    for root in _agent_roots(agents_root):
        if not root.exists():
            continue
...
def _read_agent(name: str, agents_root: Path = _BUILTIN_AGENTS_ROOT) -> ToolOutput:
    if not name.strip():
        return ToolOutput(success=False, error="'name' is required for action=read.")
    agent_dir = next(
        (root / name.strip() for root in _agent_roots(agents_root) if (root / name.strip()).exists()),
        agents_root / name.strip(),
    )
    if not agent_dir.exists():
        return ToolOutput(success=False, error=f"Agent '{name}' not found at {agent_dir}/.")
    config_text = (
