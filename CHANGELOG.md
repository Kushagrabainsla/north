...
- **TUI accent scrollbar flashed when touching the chat** (`cli/tui_v2.py`): `#log`/`#streaming` had `scrollbar-color-hover: $primary`, so hovering/clicking the chat showed an accent-coloured scrollbar - a visible change beyond the intended input-box focus border. All scrollbar substates (default/hover/active) are now the background colour, so the only focus difference is the input box border.
- **TUI now shows the active model** (`agents/agentic_llm_agent.py`, `cli/tui_v2.py`): the agent loop emits a `model` SSE event when the router picks/changes a model; the TUI shows a shortened model id in the hint bar (`cruise · llama-4-scout-17b… · ↑↓ history · ctrl+c quit`).
- **TUI no longer echoes the prompt as a `◆ general` line** (`cli/tui_v2.py`): the `agent_started` header for the default `general` agent duplicated the user's just-shown prompt; it is now suppressed (specialist agents like `coder`/`researcher` still get a header).
- **TUI redesigned to match modern coding-agent CLIs** (`cli/tui_v2.py`): top-anchored chat (no more blank-line padding that left a void above the banner and desynced on resize); a **rounded input box** (`╭─ > … ─╯`, accent border on focus) replacing the flat separators; a working-spinner line above it and a dim shortcut hint (`strategy · ↑↓ history · ctrl+c quit`) below - the blended convention shared by Claude Code / opencode / Copilot CLI.
