...
### Changed
- **Every schedule runs a flow.** `schedule_task`, `update_schedule`, `POST /orchestrator/cron` and `north cron add` take a flow, not a prompt, agent or skill; the flow must be active and unchanged since it was tested. A reminder is a flow with one step of instructions. The daily news briefing is now a built-in skill and flow, and the Schedule page is gone: schedules live on the Flows page. The nightly cleanup is a built-in flow too: its one step names a system action, deterministic server code with no model behind it, which only built-in flows may use. It shows on the Flows page with its schedule and run history like any other flow. Schedules stored before this keep running as they were.
