...
## [Unreleased]
### Added
- **The Flows page shows what a flow does and when** (`web/src/pages/Flows.tsx`, `flows/store.py`, `web/extensions.py`). Next up, your flows with their schedule and last result, built-in flows read-only, and recent runs that open to what each step did and wrote. A flow's own page manages its schedules and can test, run and activate it, so the whole life of an automation is in one place. Run history records when a run started and what started it (schedule, by hand, or test), is kept as long as task history, and a run left running by a shutdown is marked failed at the next start.
- **A fresh install starts with no schedules of its own.** North no longer seeds a daily news briefing, or ships a briefing flow or skill: a briefing is one person's routine, so whoever wants one builds it as a flow and schedules it. The `news_briefing` agent it used is unchanged, and an install that already has the schedule keeps it.
- **Deleting a flow deletes its schedules and cancels its queued runs**, after telling you which. A schedule whose flow is gone could only fail when it fired.
- **Scheduled runs prove they produced their file** (`tools/universal/flow_runner.py`, `utils/handoff.py`). A scheduled flow step whose agent declares an output, such as the news briefing, now fails when that file was not written, as the scheduled agent path always did. Runs record the files each step wrote.
- **Evidence-backed self-extension lifecycles for tools, skills, and flows.** New or edited capabilities remain candidates; tools must pass structural validation and a representative call, skills must pass positive and negative selection checks, and flows must pass live schema validation plus an exact-definition test run before explicit user-confirmed activation.
- **Built-in operating playbooks for self-improvement** (`resources/builtin-skills/`). Fresh sessions now receive dedicated procedures for authoring tools, skills, flows, specialist agents, and schedules, including reuse-first inventory, authority questions, browser preflight, bounded tests, and honest completion evidence.
- **Fixed-interval schedules.** Tasks can run every N minutes with a durable anchor, alongside existing one-shot and wall-clock schedules; the API, dashboard, persistence layer, and next-firing calculation share the same timing model.
...
### Changed
- **Every schedule runs a flow.** `schedule_task`, `update_schedule`, `POST /orchestrator/cron` and `north cron add` take a flow, not a prompt, agent or skill; the flow must be active and unchanged since it was tested. A reminder is a flow with one step of instructions. The Schedule page is gone: schedules live on the Flows page. The nightly cleanup is a built-in flow too: its one step names a system action, deterministic server code with no model behind it, which only built-in flows may use. It shows on the Flows page with its schedule and run history like any other flow. Schedules stored before this keep running as they were.
- **Flows now depend only on executable skills.** Every flow step selects one active skill and supplies instructions, bound inputs, and an approval mode. The skill's frontmatter owns its executor, exact tool allowlist, input/output schemas, minimum approval, and success criteria; the server validates those contracts, restricts both initial tool selection and `find_tools`, and rejects malformed outputs. Step approval remains server-enforced (`never` blocks mutations, `on_mutation` gates each mutation, and `always` approves the complete step), tool calls remain visible in the ledger, legacy agent/tool-step files load as candidates through a narrowly allowlisted compatibility skill, and skill edits invalidate flow run and activation fingerprints.
- **Browser automation now requires an explicit context choice.** Every browser call carries a user-confirmed isolated or existing-browser context; attaching through CDP or copying profile cookies is rejected until North explains the exposure of logged-in sessions, cookies, tabs, and extensions.
- **Mutation classification is per call.** Mixed tools such as browser and the self-extension managers classify each action from its parameters, so reads can run concurrently while clicks, edits, code loading, and activation retain approval, sequencing, and crash-recovery treatment.
- **Dashboard edits cannot bypass capability validation.** Skill, tool, and flow creates or edits are stored as candidates, including editable overrides of built-ins; active flow definitions no longer carry versions, and executable edits reset prior evidence.
- **Schedules validate live references before storage.** Unknown agents and unknown or candidate skills/flows are rejected before a timer is installed, preventing delayed failures at firing time.
- **Tools are selected by the task, not assigned to agents.** Every discovered or manually registered tool now joins one global catalog. At run start, semantic retrieval uses the task, agent role, selected skills, and current plan to inject a lean subset; explicit tool names and essential loop controls cannot be dropped, and the always-visible `find_tools` control can load a missed capability on the next turn. Agent `tools.yaml` allowlists and new-agent tool wiring were removed. Approval policy, sandboxing, workspace locks, and edit scopes still govern execution.
...
- **Source distributions no longer vendor local frontend dependencies** (`MANIFEST.in`, `.github/workflows/ci.yml`). `uv build` had included 717 `web/node_modules` entries in the sdist even though the wheel excluded them, inflating releases with reproducible local dependencies. The manifest now prunes them and CI inspects the built tarball so the exclusion remains enforced.
...
## [1.20.0] - 2026-09-04
### Changed
- **An edit now has to have read the file it edits** (`tools/_read_tracker.py`, `tools/specialized/patch_file.py`). Exact-match editing only works when the model's copy of the text is exact, and having just read the file is what makes that true. Without the precondition the model edits from memory - of a file it saw three turns and two edits ago, or never saw at all - and each miss costs a full round-trip on a model generating around twenty tokens a second. Scoped per task, so a fix round begun after the reviewer sends work back still counts as having read what the first round read; a file the agent has just written counts as read; a call outside a task is never blocked, because this is a guard rail for agents rather than a lock on the tool.
- **A remembered block at the wrong indentation is no longer a failed edit** (`tools/specialized/_edit_match.py`). When the exact bytes miss, the match is retried on the lines' content and the replacement is shifted to the file's own indentation, so the result is still correct. Uniqueness is never traded away: a tolerant match is accepted only when it is the only one, because silently editing the wrong of two similar blocks is worse than failing.
- **A failed match now shows what is actually in the file.** "old_string not found. Check exact whitespace and newlines" did not say what the whitespace *was*, so the retry was another guess. The error now carries the closest text with line numbers, and an ambiguous match names the lines it matched, so the next attempt is informed.
- The coder's work is committed after **every** round, not just the first (`orchestrator/orchestrator.py`). Committing once before the review loop left everything a fix round changed uncommitted - which is the half that matters most, since it is what the reviewer asked for.
- **Worktree isolation is on by default**, scoped to the coder as it already was (`config/settings.py`). An agent that writes code should not be doing it in the tree you are working in: a run that goes wrong is discarded with its worktree rather than needing to be undone, and two runs can never share a working tree. Falls back to editing in place when the workspace is not a git repo. Opt out with `NORTH_WORKTREE_ISOLATION_ENABLED=0`.

## [1.19.0] - 2026-09-04
### Changed
- **The coder no longer spends model turns on version control** (`orchestrator/commit.py`, `agents/coder/prompts/system.md`). Creating a branch, staging each changed file and committing need no judgement, and every one of them cost a full LLM round-trip. Measured on a two-line fix: the coder used roughly twelve turns, six of them version control, with three git approval cards raised back to back while the model decided "now add", waited, "now add again", waited, "now commit" - about a third of its 142 seconds. The orchestrator now branches and commits once the coder finishes, reading what actually changed on disk rather than asking the model to recall which files it touched, which is also more accurate. Everything still goes through `GitTool`, so the approval gate, the read-only argument allowlist and the force-push block all still apply, and generated directories are excluded - which is what the prompt's "never `git add .`" rule was guarding against.

### Fixed
- **A successful bugfix was reported as failed** (`orchestrator/dod.py`). The Definition of Done treated "the recorded reproduction passed before the fix" as proof the bug was never real. But a reviewer reaches for the existing test suite, which of course passes on a bug nothing covers yet - so a task whose fix landed, whose regression test was added, and whose reviewer verdict was PASS was still recorded `task_completed_with_failures`. That is not harmless any more: since 1.17.0 that outcome marks an episode `partial` and counts against a learned skill, so a false failure feeds straight back into what north believes about itself. The reproduction now only counts against a task when nothing else verified the change.
- `--yolo` approved only the tools that spell consent "Approve" (`cli/main.py`). git offers "Approve", patch_file "Apply", bash and shell "Run"; the CLI decided from the label and recognised only the first, so `--yolo` approved a branch checkout and then returned merely "answered" for the edit itself. "answered" is not consent, so the patch never applied - a coding run took three minutes, auto-approved everything it was asked, and left the file untouched. The mapping is a small pure function with tests now.
...
- The root log level is configurable (`log.level`, default `INFO`). It was hardcoded, which made every `logger.debug` in the codebase unreachable - including the one added in 1.16.0 to report which prompt tokens a provider served from cache.

### Fixed
- **A command nobody approved counted against the tool that asked.** 1.16.0 fixed this for `git`/`gh`, which go through `gate_mutating_action`, but `bash`, `shell` and `patch_file` take the older boolean path and were missed. Observed live: a reviewer whose approval cards went unanswered drove `bash` to 0.09 confidence in three calls, and since tool confidence orders the tool list, that also churns the prompt-cache prefix. All four now share one `refusal_output` mapping, so every gated tool reports a refusal the same way and an absent human is never recorded as a malfunction.
- The unanswered-approval cap added in 1.16.0 never fired. It reset on any batch of tool calls that raised no card, so an ordinary `read_file` between two expired cards cleared the count - observed live, where a coder raised a patch_file card, read some files, raised a git card, and sailed past the cap with nobody there to answer either. Reading a file is not evidence that a human is at the keyboard; the count now runs across the whole agent run.
- **north was teaching itself the bug.** A run that finished with failures was recorded as `outcome: "success"` (`memory/consolidator.py`), and the skill distiller learns procedures from successful episodes - so a task that told the user "I could not complete the summary" was distilled into a permanent skill whose first step read *"locate and read the required handoff file; if it is missing, stop and report that the summary cannot be verified"*. That is the 1.16.0 abort bug, written down as a reusable procedure and offered to later runs, which followed it and failed the same way. A fixed prompt was being overruled by a learned skill. `task_completed_with_failures` is now a distinct `partial` outcome: still recalled (knowing an approach half-worked is useful, and it is labelled `[PARTIAL]`), never an exemplar to learn from.
- **Every research task was told it had modified code without verifying it** (`orchestrator/result_audit.py`). The code-evidence gate fires when an engineering agent succeeds with `write_file` and runs no `check_types`/test - but the researcher and architect write handoff *documents*, which is their entire job. Correct answers were being appended with "⚠️ Unverified claims - modified code but ran no check_types or test to verify the change". Only agents that touch production code are held to the gate now. This surfaced only after 1.16.0 made the researcher actually reach the step where it writes `context.md`.
- Narration streamed before a tool call was glued to the final answer with no separator (`cli/main.py`, `cli/tui.py`), producing replies like `...pyproject.toml file.1. Minimum Python version:`. 1.16.0 fixed the self-repair variant of this; the same buffer also kept everything the model said on its way *to* a tool call. The answer is what streams after the last tool call.
- The researcher still wrote a research copy into the workspace despite being told not to - the instruction sat in a sub-branch of the workflow rather than in `## Rules`, where its hard constraints live.
- Tests wrote into the developer's real `~/.north/tasks/` (`tests/conftest.py`). `tools._path._handoff_root` reads `NORTH_HOME` from the environment, so any test that did not set it used the live home - fixture directories (`t1`, `resumed-task-1`, `t_pause_race`) were found sitting among genuine task artifacts. An autouse fixture now points `NORTH_HOME` at a temporary directory for every test and clears the caches over the resolved roots on the way in and out.

## [1.16.0] - 2026-09-03
### Fixed
- **Read-only `git` could write a file anywhere on disk, with no approval card** (`tools/specialized/git_tool.py`). `status`, `diff`, `log` and `show` are classified read-only and so skip the approval gate entirely - but their arguments were passed to the subprocess unfiltered, and git's diff family takes `--output=<path>`. `git diff --output=/anywhere` created a file outside the workspace, stepping around both the approval gate and the `tools/_path.py` sandbox; `--ext-diff` runs a command the repo configures. Arguments for read-only actions are now allowlisted rather than blocklisted: a blocklist has to predict every future git option that touches the disk and misses the first one it does not know about.
- **Every task routed to the researcher aborted before doing any work** (`agents/researcher/prompts/system.md`). Step 2 told the agent to probe for an optional resume file; a rule at the bottom told it to stop on any tool failure. It probed, `read_file` correctly answered "not found", and the agent stopped - so it never reached the step where it *writes* that file, and the next task hit the same wall. A `research/` directory had never been created, once, in 80 tasks. The probe is now gone: the orchestrator already reads predecessor artifacts into context, so the agent had no reason to look for them itself. The same trap in the architect prompt is fixed the same way.
- **An approval nobody answered was reported to the agent as "Action rejected by user"** (`tools/specialized/_approval.py`, `approval/interaction.py`). Nobody rejected anything - there was no one there. Believing it had been refused, the agent looked for another way to do the same thing, raised another card, and stalled for the full timeout again; one abandoned task kept calling the provider for twelve minutes. A timeout now says so, and `agents/agentic_llm_agent.py` stops the run after two *consecutive* unanswered cards, so a user answering slowly from Telegram resets the count and is never cut off.
- **No prompt was ever served from cache, and the reason was north's own prefix** (`agents/agentic_llm_agent.py`). A cache is a prefix match. The system prompt opened with a minute-resolution timestamp - the head of the hashed prefix for every provider - so two runs a minute apart shared nothing at all. The tool definitions, the largest stable block after it, were ordered by a confidence score that moves every time a tool succeeds or fails. The clock is now a date at the end of the prompt, and tools go on the wire sorted by name, with the confidence ranking kept as text in the reliability hints where it costs nothing. Measured on the same install: **every call before the change reported 0 cached tokens; after it, repeat calls report 10,496 of 11,151 (94%)**.
- Two answers were concatenated into one reply (`cli/main.py`, `cli/tui.py`). When the result auditor re-runs an agent to correct unverified claims, that second draft streams down the same channel as the first, and both were kept - producing replies that contradicted themselves mid-sentence (`...format_status_table()I could not complete the verification:`). `self_repair_started` was already on the wire; both clients now listen to it. A repair that starts and is never adopted falls back to the ledger, which holds the answer that was actually kept.
- `north task cancel <id>`, as documented, failed with `No such command <id>` (`cli/main.py`). `task` was a command group with a positional `prompt`, so the first word of every prompt was parsed as a subcommand name first - which also meant `-w` was only accepted *before* the prompt. It is a plain command now. Cancelling is `north cancel <id>`, which already handled tasks and jobs alike.
- `--yolo` parsed and was then ignored outside the TUI, so `north --yolo task "..."` still stopped dead at an approval prompt (`cli/main.py`).
- `git branch --list <pattern>` was gated as a mutating action (`tools/specialized/git_tool.py`). The positional is a glob to filter the listing by; a bare positional without `--list` still creates a branch and still needs approval.
- A tool that correctly reported absence was counted against it. `read_file` had decayed to the lowest-ranked tool the researcher owned (0.56, below every tool it had ever used) purely from being asked whether an optional file existed.

### Added
- **`ToolOutput.failure_kind`** (`tools/models.py`) - `error`, `not_found`, or `refused`. Three unrelated things were flattened into one `success: false` and read as the same thing by every consumer: a tool that broke, a tool that worked and answered "that is not there", and an action a human declined. Only `error` now counts as a fault. Additive with a default, so a tool that sets nothing behaves exactly as before.
- **The cockpit lists what the engineering pipeline actually produced** (`web/api.py`, `web/src/pages/Verbose.tsx`). Research notes, specs, implementation notes and QA reports live in `<NORTH_HOME>/tasks/<task_id>/` and were readable from nowhere - which is why the researcher was told to copy its findings into the user's repo as well. Artifacts is now grouped per task run, newest first, with stages in pipeline order. north's own state files share that directory, so `tasks.db` and its WAL/SHM siblings are excluded by the same suffix list the tool sandbox blocks on.
- **`handoff.retention_days`** (default 30, `config/settings.py`), swept by the existing daily `task_context_cleanup` job. Nothing ever deleted these directories: one is created per task and only losing best-of-N candidates were cleaned up, so an install accumulated one for every task it had ever run - 188 of 192 on the development machine, all empty. Empty directories go regardless of age; age is taken from the newest artifact inside, so a long task that writes its QA report late is not aged out on when it started; directories for running tasks are never touched.
- An `incomplete` column in `north metrics` (`ledger/sqlite_writer.py`, `cli/main.py`). A stage that answers but never writes the artifact it declares in `produces` has not finished its job, yet it is not a failure either - so `success_rate` called it a success. Three runs that told the user "I could not complete" were all recorded as 100%.

### Removed
- The researcher no longer writes a `<topic>-research.md` copy into the user's workspace (`agents/researcher/prompts/system.md`). It existed because the real artifact had nowhere visible to live; the cockpit is that place now.
- `north task cancel` - a shadowed duplicate of `north cancel <id>`, which was already the working command.
...
### Fixed
- **Cost tracking no longer discards a task that is still spending** (`inference/cost_tracker.py`): eviction was by insertion order, so a long-running task - registered before every newer one - was dropped first while still accumulating cost. Now least-recently-charged.
- **Delegated sub-agents inherit their parent's run framing** (`agents/agentic_llm_agent.py`): `context` (the caller's conversation history / webhook data), `model_pool`, and `exclude_models` are passed to the sub-agent instead of silently reverting to defaults.
- **Tool registry detects tools edited in place** (`tools/registry.py`): the reload check keyed on directory mtime, which only moves when a file is added or removed, so `create_tool` updating an existing tool was never picked up. The fingerprint now folds in each file's own mtime. Tool discovery also distinguishes "needs injected dependencies" from a genuine `TypeError` in a tool's own `__init__` by inspecting the constructor signature rather than catching the exception, so real bugs are logged instead of silently skipped.
- **Duplicate per-turn side-effect write** (`agents/agentic_llm_agent.py`): `_record_side_effects` ran once per successful mutating call and again for the whole turn; the flag is a per-task boolean, so once per turn is enough.
- **Skill-to-tool matching uses word boundaries** (`agents/base.py`): a substring test meant a skill merely mentioning "bash" in prose boosted the `bash` tool, and any skill containing "globally" boosted `glob`.
- **Job drain is bounded** (`jobs/sqlite_processor.py`): the poll loop claimed every pending job in one tick and spawned a task for each; it now holds at most `_MAX_CONCURRENT_JOBS` in flight, matching the cap task submission already had.
- **Redundant `conn.commit()` calls removed** (`orchestrator/task_context.py`, `memory/episodic.py`): `open_db_connection` already commits on clean exit, and committing inside the block would release an enclosing savepoint early if these were ever called nested.

### Changed
- **Blocking filesystem I/O moved off the event loop** (`orchestrator/orchestrator.py`, `orchestrator/worktree.py`, `web/api.py`, `bootstrap/onboarding.py`, `tools/specialized/north_config.py`, `tools/universal/create_skill.py`): 22 filesystem calls inside `async def` bodies now run via `asyncio.to_thread` per CODING_STYLE §10.3. The ones that mattered: `verify_claims()` stats claimed paths after every agent result; best-of-N promoted the winning handoff directory with `shutil.copytree`/`rmtree`; `GET /web/api/system` walked the user's home directory via `_discover_files()`; and `POST /web/api/providers/{id}` read and rewrote `~/.north/.env` inline.
- **Bootstrap no longer re-hashes every scanned file on every step** (`bootstrap/onboarding.py`): the progress checkpoint is rebuilt after each file and recomputed a SHA-256 over *all* files each time, making a scan O(files²) in bytes read; fingerprints are now computed once per file and cached (12 files: 78 hashes → 12). `_extract_unified` likewise hashed the source file once per extracted fact rather than once per file.
