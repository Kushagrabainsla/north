    ...
    >
      ...
      <label>
        ...
        <textarea
          value={draft.task}
          rows={3}
          placeholder="e.g. remind me to stretch and log it"
      ...
      <div className="schedule-form-actions">
        <button
          type="submit"
          className="primary-button"
          disabled={busy || !draft.task.trim() || noDays}
        >
...
function BuiltinRow({
  ...
  const facts: [string, string][] = [
    ...
    [
      "Next run",
      entry.enabled
        ? `${entry.next_run_local} (${whenFromNow(entry.next_run_epoch)})`
        : "paused",
    ],
    [
      "Settings",
      entry.modified
        ? "changed from the ones north ships with"
        : "as north ships them",
    ],
  ];
  return (
    <div
      className={
        entry.enabled ? "schedule-row builtin" : "schedule-row builtin paused"
      }
    >
      <div className="schedule-main">
        ...
        {entry.description && (
          <small className="schedule-description">{entry.description}</small>
        )}
        <small>
          {entry.interval_minutes
            ? entry.cadence
...
export function Schedule() {
  ...
  const save = (name: string) =>
    act(() => patch(`/orchestrator/cron/${encodeURIComponent(name)}`, body()));
  const setEnabled = (entry: Cron, enabled: boolean) =>
    act(() =>
      patch(`/orchestrator/cron/${encodeURIComponent(entry.name)}`, {
        enabled,
      }),
    );
  ...
  const cancelJob = (job: Job) =>
    act(() => del(`/orchestrator/jobs/${encodeURIComponent(job.job_id)}`));
  ...
  const titleOf = (job: Job) =>
    job.label ||
    routines.find((entry) => entry.name === job.cron_entry)?.title ||
    job.task;
  ...
  const busyRoutines = new Set(
    [...running, ...queued].map((job) => job.cron_entry).filter(Boolean),
  );
  ...
  const upcoming = [
    ...running.map((job) => ({
      ...
      kind: "running now" as const,
      job: undefined,
    })),
    ...queued
      .filter((job) => job.cron_entry)
      .map((job) => ({
        key: job.job_id,
        task: titleOf(job),
        agent: job.agent,
    ...
    ...routines
      .filter((entry) => entry.enabled && !busyRoutines.has(entry.name))
      .map((entry) => ({
        key: entry.name,
        task: entry.title,
        agent: entry.agent,
  ...
  return (
    <div className="page">
      ...
      <PageHeader
        eyebrow="Automation"
        title="Schedule"
        subtitle="What north will do next, and the routines behind it."
        actions={
      ...
      {creating && (
        <Panel
          title={draft.repeat === "once" ? "New one-off" : "New routine"}
          label="new"
        >
          <ScheduleForm
            draft={draft}
            ...
            onCancel={close}
            submitLabel={
              draft.repeat === "once" ? "Schedule it" : "Create routine"
            }
            busy={busy}
            error={error}
            agents={agents}
      ...
      {(cron.error || jobs.error) && (
        <ErrorNotice message={cron.error || jobs.error} />
      )}

      <Panel title="Next up" label="soonest first">
        {cron.loading || jobs.loading ? (
          <Loading />
        ) : upcoming.length ? (
            ...
            <div className="schedule-row upcoming" key={item.key}>
              <div className="schedule-main">
                ...
                <small title={item.absolute}>
                  {whenFromNow(item.epoch)} · {item.absolute} · {item.kind} ·{" "}
                  {item.agent}
                </small>
              </div>
              {item.job ? (
                confirming === item.key ? (
                  <ConfirmRow
                    question="Cancel this one-off?"
                    confirmLabel="Cancel it"
                    busy={busy}
                    onConfirm={() => cancelJob(item.job!)}
          ...
          <Empty>
            Nothing is scheduled. Add a routine below, or ask north to remind
            you about something.
          </Empty>
        )}
      </Panel>
      ...
      <Panel title="Your routines" label={`${mine.length} recurring`}>
        {cron.loading ? (
            ...
            >
              ...
              {confirming === entry.name ? (
                <ConfirmRow
                  busy={busy}
                  onCancel={() => setConfirming("")}
                  onConfirm={() => removeOrRestore(entry)}
                  question="Delete this routine? It will not run again."
                  confirmLabel="Delete"
                />
              ) : (
                <div className="schedule-actions">
          ...
          <Empty>
            No routines of your own yet. Schedule something above, or just ask
            north for it.
          </Empty>
        )}
      </Panel>
                ...
                confirming === entry.name && (
                  <div className="schedule-restore">
                    <ConfirmRow
                      ...
                      onCancel={() => setConfirming("")}
                      onConfirm={() => removeOrRestore(entry)}
                      question="Put this back to the settings north ships with?"
...
export function Flows() {
  ...
  const completeSteps = Boolean(
    draft?.steps.every((step) => step.name.trim() && step.instructions.trim()),
  );
    ...
    <section className="flow-builder">
      <div className="flow-builder-intro">
        <div>
          ...
          <p>
            Arrange existing skills in order, or write inline instructions when
            a step does not need a reusable skill. Approval is inherited from
            the selected skill and system policy.
          </p>
        </div>
      ...
      <div className="flow-steps-heading">
        <div>
          <h2>Steps</h2>
          <p>
            North executes each selected skill or inline procedure from top to bottom.
          </p>
        </div>
        <button
          className="ghost-button"
  ...
  return (
    <div className="page capability-page">
      <PageHeader
        eyebrow="Automation"
        title="Flows"
        subtitle="Design the ordered processes North can run."
        actions={
          <button className="primary-button" onClick={startCreating}>
            + New flow
          </button>
      ...
      />
      {flows.error && <ErrorNotice message={flows.error} />}{" "}
      {message && <div className="notice">{message}</div>}
      {creating && (
      ...
      <section className="capability-registry">
        ...
        <div className="capability-registry-list">
          ...
          {!flows.loading && !flows.data?.length && (
            <Empty>No flows have been created yet.</Empty>
          )}
        </div>
      </section>
      <CapabilityModal
