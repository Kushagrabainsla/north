...
import { Empty, ErrorNotice, Loading, Markdown, PageHeader, Panel, Status, timeAgo } from "../components";
...
const docs = ["user", "north_stars", "judgement_rules", "soul"];
...
export function Memory() {
  ...
  return <div className="page"><PageHeader eyebrow="Knowledge" title="Memory" subtitle="The durable context North brings into your work." actions={<button className="primary-button" disabled={draft === null} onClick={save}>Save document</button>}/><div className="memory-layout"><aside>{docs.map(name => <button className={doc === name ? "active" : ""} key={name} onClick={() => { setDoc(name); setDraft(null); }}>{name.replaceAll("_", " ")}.md</button>)}</aside><div className="memory-editor-grid"><section><div className="editor-label">Markdown source</div>{resource.loading ? <Loading/> : <textarea className="document-editor" value={content} onChange={e => setDraft(e.target.value)} />}</section><section className="memory-preview"><div className="editor-label">Rendered preview</div><Markdown>{content || "Nothing written yet."}</Markdown></section></div></div><Bootstrap embedded/></div>;
}
...
export function SystemPage() {
  const overview = useResource<any>("/web/api/system", 8000); const metrics = useResource<any>("/orchestrator/metrics?days=30", 15000); const costs = useResource<any>("/orchestrator/inference/costs?period=month", 15000); const models = useResource<Record<string, any>>("/orchestrator/inference/models", 15000); const health = useResource<{status:string}>("/health", 5000);
