...
function CapabilityModal({ open, title, eyebrow, actions, className = "", onClose, children }: {
  open: boolean;
  title: string;
  eyebrow: string;
  actions?: ReactNode;
...
export function Skills() {
  ...
  const startCreating = () => { setSelected(null); setDraft(emptySkill()); setMessage(""); setCreating(true); };
  const open = async (name: string) => { setSelected(name); setCreating(false); setDetailLoading(true); setContent(""); setMessage(""); try { const detail = await api<SkillDetail>(`/web/api/skills/${encodeURIComponent(name)}`); setContent(detail.content); } catch (error) { setSelected(null); setMessage(error instanceof Error ? error.message : String(error)); } finally { setDetailLoading(false); } };
  ...
  return <div className="page capability-page"><PageHeader eyebrow="Procedures" title="Skills" subtitle="Inspect and edit the playbooks North injects into specialist work." actions={<button className="primary-button" onClick={startCreating}>+ New skill</button>}/>{skills.error && <ErrorNotice message={skills.error}/>} {message && <div className="notice">{message}</div>}
    ...
    <CapabilityModal open={Boolean(selected)} onClose={closeEditor} title={selected || "Skill"} eyebrow={selectedSkill?.source || "Procedure"}
      ...
      {detailLoading ? <Loading/> : <section className="skill-editor modal-editor"><div className="editor-label"><span>{selected}/SKILL.md</span><span>Markdown instructions {selectedSkill?.source === "builtin" && "· read-only"}</span></div><textarea aria-label={`${selected} Markdown skill instructions`} readOnly={selectedSkill?.source === "builtin"} value={content} onChange={event => setContent(event.target.value)}/></section>}
    </CapabilityModal>
  </div>;
}
...
export function Flows() {
  ...
  return <div className="page capability-page"><PageHeader eyebrow="Automation" title="Flows" subtitle="Design the ordered, approval-aware processes North can run." actions={<button className="primary-button" onClick={startCreating}>+ New flow</button>}/>{flows.error && <ErrorNotice message={flows.error}/>} {message && <div className="notice">{message}</div>}
    {creating && <section className="capability-create-panel flow-create-panel"><header><div><span>New automation</span><h2>Create flow</h2><p>Define the outcome and the ordered steps before adding it to the registry.</p></div><div><button className="ghost-button" onClick={() => { setCreating(false); setDraft(null); }}>Cancel</button><button className="primary-button" disabled={!canSave} onClick={() => void create()}>Create flow</button></div></header>{flowEditor}</section>}
    <section className="capability-registry">
      <div className="capability-registry-head"><span>Flow</span><span>Outcome</span><span>Sequence</span><span>Status</span><span/></div>
      <div className="capability-registry-list">{flows.loading ? <Loading/> : (flows.data || []).map(flow => <button className="capability-registry-row" key={flow.name} aria-haspopup="dialog" onClick={() => void open(flow.name)}><span className="capability-registry-name"><b>{flow.name}</b><small>{flow.source}</small></span><span className="capability-registry-description">{flow.description}</span><span className="capability-registry-meta">v{flow.version} · {flow.steps} step{flow.steps === 1 ? "" : "s"}</span><Status value={flow.status}/><span className="capability-registry-open">Open</span></button>)}{!flows.loading && !flows.data?.length && <Empty>No flows have been created yet.</Empty>}</div>
    </section>
    <CapabilityModal open={Boolean(selected)} onClose={closeEditor} title={selected || "Flow"} eyebrow={selectedFlow?.source || "Automation"} className="flow-modal"
      actions={<>{selectedFlow?.source === "learned" && <button className="danger-button" onClick={() => void remove()}>Delete flow</button>}<button className="primary-button" disabled={!canSave} onClick={() => void save()}>Save flow</button></>}>
      {flowEditor}
    </CapabilityModal>
  </div>;
}
...
export function Tools() {
  ...
  return <div className="page capability-page"><PageHeader eyebrow="Capabilities" title="Tools" subtitle="Inspect the atomic operations North can call, and manage your learned tools." actions={<button className="primary-button" onClick={startCreating}>+ New tool</button>}/>{tools.error && <ErrorNotice message={tools.error}/>} {message && <div className="notice">{message}</div>}
    ...
    <CapabilityModal open={Boolean(selected)} onClose={closeEditor} title={selected || "Tool"} eyebrow={selectedTool?.source || "Capability"}
      ...
      {detailLoading ? <Loading/> : <section className="skill-editor tool-editor modal-editor"><div className="editor-label">{selected}.py {selectedTool?.source === "builtin" && <span className="editor-lock">read-only</span>}</div><textarea aria-label={`${selected} tool implementation`} readOnly={selectedTool?.source === "builtin"} value={content} onChange={event => setContent(event.target.value)}/></section>}
    </CapabilityModal>
  </div>;
}
