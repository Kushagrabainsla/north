...
export function Skills() {
  ...
  return <div className="page capability-page"><PageHeader eyebrow="Procedures" title="Skills" subtitle="Inspect and edit the playbooks North injects into specialist work." actions={<button className="primary-button" onClick={startCreating}>+ New skill</button>}/>{skills.error && <ErrorNotice message={skills.error}/>} {message && <div className="notice">{message}</div>}
    ...
    <CapabilityModal open={Boolean(selected)} onClose={closeEditor} title={selected || "Skill"} eyebrow={selectedSkill?.source || "Procedure"}
      ...
      {detailLoading ? <Loading/> : <section className="skill-preview-layout"><div className="skill-editor-pane"><div className="editor-label"><span>{selected}/SKILL.md</span><span>{selectedSkill?.source === "builtin" ? "Built-in baseline · editable override" : "Markdown instructions · editable"}</span></div><ContentTextarea aria-label={`${selected} Markdown skill instructions`} value={content} onChange={event => setContent(event.target.value)}/></div><aside className="skill-preview-pane"><div className="editor-label">Rendered Markdown</div><div className="skill-preview-content"><Markdown>{content || "Nothing written yet."}</Markdown></div></aside></section>}
    </CapabilityModal>
  </div>;
}
...
export function Tools() {
  ...
  return <div className="page capability-page"><PageHeader eyebrow="Capabilities" title="Tools" subtitle="Inspect the atomic operations North's skills can call, and manage your learned tools." actions={<button className="primary-button" onClick={startCreating}>+ New tool</button>}/>{tools.error && <ErrorNotice message={tools.error}/>} {message && <div className="notice">{message}</div>}
    ...
    <CapabilityModal open={Boolean(selected)} onClose={closeEditor} title={selected || "Tool"} eyebrow={selectedTool?.source || "Capability"}
      actions={selected ? <><button className="primary-button" disabled={detailLoading} onClick={() => void save()}>Save tool</button>{selectedTool?.source === "learned" && <button className="danger-button" onClick={() => void remove()}>Delete tool</button>}</> : undefined}>
      {detailLoading ? <Loading/> : <section className="tool-preview-layout"><div className="skill-editor-pane tool-editor"><div className="editor-label"><span>{selected}.py</span><span>Python implementation · editable</span></div><ContentTextarea aria-label={`${selected} Python implementation`} value={content} onChange={event => setContent(event.target.value)}/></div><aside className="tool-preview-pane"><div className="editor-label">Python preview</div><pre><code>{content || "# Nothing implemented yet."}</code></pre></aside></section>}
    </CapabilityModal>
  </div>;
}
