...
export function Agents() {
  ...
  const remove = async (agent: Agent) => { const linkedSchedules = (schedules.data || []).filter(item => item.agent === agent.name); const linkedJobs = (pendingJobs.data || []).filter(item => item.agent === agent.name); if (!agent.deletable || !cascadeReady || !await dialog.confirm(agentDeletionPrompt(agent.name, linkedSchedules.map(item => item.title), linkedJobs.length), { title: "Delete personal agent?", confirmLabel: "Delete agent", danger: true })) return; try { await del(`/orchestrator/agents/${encodeURIComponent(agent.name)}`); setMessage(`Deleted personal agent '${agent.name}' and its future work.`); await Promise.all([agents.reload(), schedules.reload(), pendingJobs.reload()]); } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); } };
...
export function Skills() {
  ...
  const [message, setMessage] = useState("");
  const open = async (name: string) => { try { const detail = await api<SkillDetail>(`/web/api/skills/${encodeURIComponent(name)}`); setSelected(name); setCreating(false); setContent(detail.content); setMessage(""); } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); } };
  const selectedSkill = skills.data?.find(skill => skill.name === selected);
  const create = async () => {
    if (!draft.name.trim() || !draft.description.trim() || !draft.instructions.trim()) return;
    try {
  ...
  return <div className="page"><PageHeader eyebrow="Procedures" title="Skills" subtitle="Inspect and edit the playbooks North injects into specialist work." actions={<><button className="ghost-button" onClick={() => { setCreating(value => !value); setMessage(""); }}>+ New skill</button><button className="primary-button" disabled={!selected || selectedSkill?.source === "builtin"} onClick={save}>Save skill</button>{selectedSkill?.source === "learned" && <button className="danger-button" onClick={() => void remove()}>Delete skill</button>}</>}/>{skills.error && <ErrorNotice message={skills.error}/>} {message && <div className="notice">{message}</div>}
    {creating && <section className="tool-create skill-create panel"><header><div><span>New procedure</span><h2>Create skill</h2></div></header><div className="skill-create-grid"><label>Name<input autoFocus value={draft.name} placeholder="review-job-match" onChange={event => setDraft({ ...draft, name: event.target.value })}/></label><label>Domains<input value={draft.domains} placeholder="jobs, research" onChange={event => setDraft({ ...draft, domains: event.target.value })}/></label><label className="skill-create-wide">Description<input value={draft.description} placeholder="When should North use this skill?" onChange={event => setDraft({ ...draft, description: event.target.value })}/></label><label className="skill-create-wide">Instructions<textarea rows={6} value={draft.instructions} placeholder="Write the ordered procedure North should follow." onChange={event => setDraft({ ...draft, instructions: event.target.value })}/></label></div><div className="tool-create-footer"><small>Use a lowercase, hyphenated name. Domains are comma-separated.</small><button className="primary-button" disabled={!canCreate} onClick={() => void create()}>Create skill</button></div></section>}
    <div className="skills-layout"><div className="skill-library">{skills.loading ? <Loading/> : (skills.data || []).map(skill => <button className={selected === skill.name ? "active" : ""} key={skill.name} onClick={() => open(skill.name)}><b>{skill.name}</b><small>{skill.source} · v{skill.version} · {skill.domains.join(", ")}</small><p>{skill.description}</p></button>)}{!skills.loading && !skills.data?.length && <Empty>No skills have been created yet.</Empty>}</div><section className="skill-editor">{selected ? <><div className="editor-label">{selected}/SKILL.md {selectedSkill?.source === "builtin" && <span className="editor-lock">read-only</span>}</div><textarea aria-label={`${selected} skill definition`} readOnly={selectedSkill?.source === "builtin"} value={content} onChange={event => setContent(event.target.value)}/></> : <div className="capability-empty-state"><div><span className="eyebrow">Skill library</span><h2>{skills.data?.length ? "Select a skill" : "Create your first skill"}</h2><p>{skills.data?.length ? "Choose a playbook from the ledger to inspect its definition." : "Skills are reusable procedures that agents can apply inside tasks and flows."}</p>{!skills.data?.length && <button className="primary-button" onClick={() => setCreating(true)}>Create skill</button>}</div></div>}</section></div>
  </div>;
}
...
interface FlowDetail { name: string; description: string; version: string; status: string; domains: string[]; source: string; steps: FlowStepDraft[]; }
interface FlowDraft { name: string; description: string; version: string; status: string; domains: string[]; steps: FlowStepDraft[]; }

const emptyStep = (): FlowStepDraft => ({ name: "", tool: "", skill: "", approval: "on_mutation", description: "", params: {} });
const emptyFlow = (): FlowDraft => ({ name: "", description: "", version: "1.0.0", status: "active", domains: ["general"], steps: [emptyStep()] });
const draftFromDetail = (detail: FlowDetail): FlowDraft => ({ name: detail.name, description: detail.description, version: detail.version, status: detail.status, domains: detail.domains, steps: detail.steps.map(step => ({ ...emptyStep(), ...step, params: step.params || {} })) });

function FlowStepCard({ step, index, onChange, onRemove, canRemove, editable }: { step: FlowStepDraft; index: number; onChange: (step: FlowStepDraft) => void; onRemove: () => void; canRemove: boolean; editable: boolean }) {
...
export function Flows() {
  ...
  const open = async (name: string) => { try { const detail = await api<FlowDetail>(`/web/api/flow-definitions/${encodeURIComponent(name)}`); setSelected(name); setCreating(false); setDraft(draftFromDetail(detail)); setMessage(""); } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); } };
  ...
  const editable = creating || selectedFlow?.source !== "builtin";
  return <div className="page"><PageHeader eyebrow="Automation" title="Flows" subtitle="Design the ordered, approval-aware processes North can run." actions={<><button className="ghost-button" onClick={startCreating}>+ New flow</button><button className="primary-button" disabled={!draft || !draft.name.trim() || !draft.description.trim() || !draft.steps.length || !editable} onClick={() => void (creating ? create() : save())}>{creating ? "Create flow" : "Save flow"}</button>{selectedFlow?.source === "learned" && <button className="danger-button" onClick={() => void remove()}>Delete flow</button>}</>}/>{flows.error && <ErrorNotice message={flows.error}/>} {message && <div className="notice">{message}</div>}<div className="skills-layout"><div className="skill-library">{flows.loading ? <Loading/> : (flows.data || []).map(flow => <button className={selected === flow.name ? "active" : ""} key={flow.name} onClick={() => void open(flow.name)}><b>{flow.name}</b><small>{flow.source} · v{flow.version} · {flow.steps} steps</small><p>{flow.description}</p></button>)}{!flows.loading && !flows.data?.length && <Empty>No flows have been created yet.</Empty>}</div><section className="flow-builder">{draft ? <><div className="flow-builder-intro"><div><div className="editor-label">Flow details</div><p>Give the flow a clear outcome, then arrange the steps North should take. Use approval checkpoints for external or consequential actions.</p></div><span className="flow-source-badge">{creating ? "new flow" : selectedFlow?.source}</span></div><div className="flow-meta-grid"><label>Flow name<input disabled={!creating} value={draft.name} placeholder="job-application-review" onChange={event => setDraft({ ...draft, name: event.target.value })}/></label><label>Version<input value={draft.version} disabled={!editable} onChange={event => setDraft({ ...draft, version: event.target.value })}/></label><label>Status<select aria-label="Status" value={draft.status} disabled={!editable} onChange={event => setDraft({ ...draft, status: event.target.value })}><option value="active">Active</option><option value="candidate">Draft / candidate</option><option value="retired">Retired</option></select></label></div><label className="flow-wide-field">Outcome<textarea disabled={!editable} rows={2} value={draft.description} placeholder="What should this flow accomplish?" onChange={event => setDraft({ ...draft, description: event.target.value })}/></label><div className="flow-steps-heading"><div><h2>Steps</h2><p>North runs these in order from top to bottom.</p></div><button className="ghost-button" disabled={!editable} onClick={() => setDraft({ ...draft, steps: [...draft.steps, emptyStep()] })}>+ Add step</button></div>{draft.steps.map((step, index) => <FlowStepCard key={`${index}-${step.name}`} step={step} index={index} editable={editable} canRemove={editable && draft.steps.length > 1} onRemove={() => setDraft({ ...draft, steps: draft.steps.filter((_, stepIndex) => stepIndex !== index) })} onChange={next => setDraft({ ...draft, steps: draft.steps.map((current, stepIndex) => stepIndex === index ? next : current) })}/>)}</> : <div className="capability-empty-state"><div><span className="eyebrow">Flow library</span><h2>{flows.data?.length ? "Select a flow" : "Create your first flow"}</h2><p>{flows.data?.length ? "Choose an automation from the ledger to inspect its ordered steps." : "Flows combine tools and skills into an approval-aware process North can run."}</p>{!flows.data?.length && <button className="primary-button" onClick={startCreating}>Create flow</button>}</div></div>}</section></div></div>;
}

interface ToolSummary { name: string; description: string; source: string; status: string; mutating: boolean; }
interface ToolDetail extends ToolSummary { content: string; }
...
export function Tools() {
  ...
  const open = async (toolName: string) => { try { const detail = await api<ToolDetail>(`/web/api/tools/${encodeURIComponent(toolName)}`); setSelected(toolName); setCreating(false); setContent(detail.content); setMessage(""); } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); } };
  const create = async () => { if (!name.trim() || !description.trim()) return; try { const created = await post<ToolSummary>("/web/api/tools", { name: name.trim(), description: description.trim() }); setName(""); setDescription(""); setCreating(false); await tools.reload(); await open(created.name); setMessage("Tool created and loaded."); } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); } };
  const save = async () => { if (!selected || selectedTool?.source !== "learned") return; try { await api(`/web/api/tools/${encodeURIComponent(selected)}`, { method: "PUT", body: JSON.stringify({ content }) }); setMessage("Tool saved and reloaded."); await tools.reload(); } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); } };
  ...
  return <div className="page"><PageHeader eyebrow="Capabilities" title="Tools" subtitle="Inspect the atomic operations North can call, and manage your learned tools." actions={<><button className="ghost-button" onClick={() => { setCreating(!creating); setMessage(""); }}>+ New tool</button><button className="primary-button" disabled={!selected || selectedTool?.source !== "learned"} onClick={() => void save()}>Save tool</button>{selectedTool?.source === "learned" && <button className="danger-button" onClick={() => void remove()}>Delete tool</button>}</>}/>{tools.error && <ErrorNotice message={tools.error}/>} {message && <div className="notice">{message}</div>}{creating && <section className="tool-create panel"><header><div><span>New capability</span><h2>Create tool</h2></div></header><div className="tool-create-grid"><label>Name<input autoFocus value={name} placeholder="search_jobs" onChange={event => setName(event.target.value)}/></label><label>Description<input value={description} placeholder="What should this operation do?" onChange={event => setDescription(event.target.value)}/></label></div><div className="tool-create-footer"><small>North creates a safe implementation stub. You can implement it in the editor after creation.</small><button className="primary-button" onClick={() => void create()}>Create tool</button></div></section>}<div className="skills-layout tools-layout"><div className="skill-library">{tools.loading ? <Loading/> : (tools.data || []).map(tool => <button className={selected === tool.name ? "active" : ""} key={tool.name} onClick={() => void open(tool.name)}><b>{tool.name}</b><small>{tool.source} · {tool.mutating ? "mutating" : "read-only"}</small><p>{tool.description}</p></button>)}{!tools.loading && !tools.data?.length && <Empty>No tools are registered yet.</Empty>}</div><section className="skill-editor tool-editor">{selected ? <><div className="editor-label">{selected}.py {selectedTool?.source === "builtin" && <span className="editor-lock">read-only</span>}</div><textarea aria-label={`${selected} tool implementation`} readOnly={selectedTool?.source === "builtin"} value={content} onChange={event => setContent(event.target.value)}/></> : <div className="capability-empty-state"><div><span className="eyebrow">Tool registry</span><h2>{tools.data?.length ? "Select a tool" : "Create your first tool"}</h2><p>{tools.data?.length ? "Choose an operation from the ledger to inspect its implementation." : "Tools are the atomic operations agents and flows can call."}</p>{!tools.data?.length && <button className="primary-button" onClick={() => setCreating(true)}>Create tool</button>}</div></div>}</section></div></div>;
}
