...
export function SystemPage() {
  ...
  const [keys, setKeys] = useState<Record<string, string>>({}); const [providerMessage, setProviderMessage] = useState("");
  const providers = overview.data?.providers || []; const totalCost = Number(costs.data?.total_cost_usd || 0);
...
export function Bootstrap() {
  ...
  return <div className="page"><PageHeader eyebrow="Knowledge" title="Bootstrap" subtitle="See what North has learned, what it has read, and choose eligible documents for another bootstrap pass." actions={<button className="primary-button" onClick={start}>Bootstrap selected</button>}/><div className="metric-cards"><div><span>Status</span><strong>{bootstrap?.status || "–"}</strong></div><div><span>Eligible documents</span><strong>{bootstrap?.candidate_count || 0}</strong></div><div><span>Completed documents</span><strong>{completed.size}</strong></div></div>{message && <div className="notice">{message}</div>}<Panel title="Eligible documents" label="Select sources"><div className="bootstrap-list">{candidates.map((path: string) => <label className="bootstrap-row" key={path}><input type="checkbox" checked={selected.includes(path)} onChange={event => setSelected(current => event.target.checked ? [...current, path] : current.filter(item => item !== path))}/><span><b>{path.split("/").pop()}</b><small>{path}</small></span><Status value={completed.has(path) ? "completed" : "queued"}/></label>)}</div>{!candidates.length && <Empty>No eligible bootstrap documents found.</Empty>}</Panel></div>;
}
