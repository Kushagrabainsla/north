...
export function Memory() {
  ...
  return <div className="page"><PageHeader eyebrow="Knowledge" title="Memory" subtitle="The durable context North brings into your work." actions={<button className="primary-button" disabled={draft === null} onClick={save}>Save document</button>}/><div className="memory-layout"><aside>{docs.map(name => <button className={doc === name ? "active" : ""} key={name} onClick={() => { setDoc(name); setDraft(null); }}>{name.replaceAll("_", " ")}.md</button>)}</aside><div className="memory-editor-grid"><section><div className="editor-label">Markdown source</div>{resource.loading ? <Loading/> : <textarea className="document-editor" value={content} onChange={e => setDraft(e.target.value)} />}</section><section className="memory-preview"><div className="editor-label">Rendered preview</div><Markdown>{content || "Nothing written yet."}</Markdown></section></div></div><section className="facts-panel panel"><header><div><span>Durable context</span><h2>Facts</h2></div><span>{facts.data?.length || 0} stored</span></header><div className="panel-content"><div className="fact-list">{facts.loading ? <Loading/> : (facts.data || []).map(fact => <article className="fact-row" key={fact.id}><div><b>{fact.content}</b><small>{fact.category || "general"} · updated {timeAgo(fact.updated_at)}</small></div><span>{fact.confidence != null ? `${Math.round(Number(fact.confidence) * 100)}%` : "Active"}</span></article>)}{!facts.loading && !facts.data?.length && <Empty>No durable facts have been captured yet.</Empty>}</div></div></section><Bootstrap embedded/></div>;
}
