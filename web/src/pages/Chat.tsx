    ...
    <div className="north-response"><div className="avatar north-avatar">N</div><div className="response-body">
      ...
      <details className="turn-detail-drawer"><summary><span>Details</span><small>{agents.length || "–"} agents · {tools.length} tool events</small><i>⌄</i></summary><div><div className="turn-summary"><span>{runs.reduce((n, r) => n + r.tokens_in + r.tokens_out, 0).toLocaleString()} tokens</span><span>${cost.toFixed(4)}</span></div><div className="execution-details">
        ...
        <DetailSection title="Agent runs" count={runs.length}>
          {runs.length ? runs.map(run => <div className="run-card" key={run.run_id}><div><b>{run.agent}</b><Status value={run.status}/></div><small>Attempt {run.attempt + 1} · {run.duration_ms ? `${(run.duration_ms / 1000).toFixed(1)}s` : "running"} · {run.providers_used?.join(", ") || "provider pending"} · {run.models_used.join(", ") || "model pending"}</small>{run.prompt_profiles?.length ? <small>{run.prompt_profiles.length} model turns · prompt: {Object.entries(run.prompt_profiles.at(-1)?.sections || {}).filter(([, tokens]) => tokens > 0).map(([name, tokens]) => `${name.replaceAll("_", " ")} ${tokens.toLocaleString()}`).join(" · ")}</small> : null}{run.error && <p className="error-text">{run.error}</p>}</div>) : <Empty>No agent runs recorded yet.</Empty>}
        </DetailSection>
