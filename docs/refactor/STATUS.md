...
| Enforce task edit scopes in source-mutating tools | In progress | `e704500` | Full pytest — 1,977 passed, 3 skipped; ruff format/check; architecture mypy — no issues in 4 files | Server-owned scopes propagate through task and agent payloads to `write_file` and `patch_file`; denied edits fail before mutation. Multi-file rename and direct-tool paths remain to be covered before this Stage 2 item is complete. |
