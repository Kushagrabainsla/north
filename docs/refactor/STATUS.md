...
| Enforce task edit scopes in source-mutating tools | Ready to commit | Pending | Full pytest — 1,977 passed, 3 skipped; ruff format/check; architecture mypy — no issues in 4 files | Server-owned scopes propagate through task and agent payloads to `write_file` and `patch_file`; denied edits fail before mutation. |
