...
north handles personal user data: context documents, ledger entries, scheduled tasks, and voice audio in transit to OpenRouter. The local data store at `~/.north/` never leaves the machine. The shared secret in `~/.north/secret.key` protects the local Orchestrator REST API and notification callbacks.
...
- Reading the context documents under `~/.north/` or `~/.north/secret.key` from outside the local user account.
