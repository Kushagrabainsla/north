...
`detect_verify_command` must only ever return a FIXED literal command derived
...
def test_detects_go(tmp_path: Path) -> None:
    ...
    assert detect_verify_command(str(tmp_path)) == "go test ./..."


def test_detects_cargo(tmp_path: Path) -> None:
    (tmp_path / "Cargo.toml").write_text("[package]\nname = 'x'\n", encoding="utf-8")
    assert detect_verify_command(str(tmp_path)) == "cargo test"
...
def test_missing_workspace_returns_none() -> None:
    assert detect_verify_command("") is None
    assert detect_verify_command("/nonexistent/path/xyz") is None
