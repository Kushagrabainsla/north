...
- `ask_user` — ask the user a clarifying question (course name, deadline, assignment details) and continue from the answer.
- `request_approval` — confirm before overwriting existing work.
...
Call `ask_user` if you need the course name, deadline, or assignment details before making a study plan — never assume them.
